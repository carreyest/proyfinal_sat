<?php
//Include Common Files @1-536757C9
define("RelativePath", ".");
define("PathToCurrentPage", "/");
define("FileName", "periodos.php");
...
//Class_Initialize Event @3-6E26805C
    function clsGridperiodos_carga($RelativePath, & $Parent)
    {
        ...
        $this->periodos_carga_Insert->Page = "periodos.php";
...
//DataSource Variables @3-30CA59C6
    public $Parent = "";
    public $CCSEvents = "";
    public $CCSEventResult;
    public $ErrorBlock;
...
//DataSourceClass_Initialize Event @3-9F571D70
...
//Prepare Method @3-FC6717A7
    function Prepare()
    {
        ...
        $this->wp->AddParameter("4", "urls_tipo_periodo", ccsText, "", "", $this->Parameters["urls_tipo_periodo"], "", false);
    }
//End Prepare Method

//Open Method @3-CF35893B
    function Open()
    {
        $this->CCSEventResult = CCGetEvent($this->CCSEvents, "BeforeBuildSelect", $this->Parent);
        $this->CountSQL = "SELECT COUNT(*) FROM (SELECT id_periodo, num_intentos_permitidos, anio, CONVERT(datetime ,fecha_limite,105) as  fecha_limite, tipo_periodo, id_proveedor, nombre_mes, mes \n" .
        "FROM periodos_carga\n" .
        "WHERE (id_proveedor = " . $this->SQLValue($this->wp->GetDBValue("1"), ccsInteger) . " or  0= " . $this->SQLValue($this->wp->GetDBValue("1"), ccsInteger) . ")\n" .
        "AND (anio = " . $this->SQLValue($this->wp->GetDBValue("2"), ccsInteger) . " or  " . $this->SQLValue($this->wp->GetDBValue("2"), ccsInteger) . "='' )\n" .
        "AND nombre_mes LIKE '%" . $this->SQLValue($this->wp->GetDBValue("3"), ccsText) . "%'\n" .
        "AND tipo_periodo LIKE '%" . $this->SQLValue($this->wp->GetDBValue("4"), ccsText) . "%' ) cnt";
        $this->SQL = "SELECT id_periodo, num_intentos_permitidos, anio, CONVERT(datetime ,fecha_limite,105) as  fecha_limite, tipo_periodo, id_proveedor, nombre_mes, mes \n" .
        "FROM periodos_carga\n" .
        "WHERE (id_proveedor = " . $this->SQLValue($this->wp->GetDBValue("1"), ccsInteger) . " or  0= " . $this->SQLValue($this->wp->GetDBValue("1"), ccsInteger) . ")\n" .
        "AND (anio = " . $this->SQLValue($this->wp->GetDBValue("2"), ccsInteger) . " or  " . $this->SQLValue($this->wp->GetDBValue("2"), ccsInteger) . "='' )\n" .
        "AND nombre_mes LIKE '%" . $this->SQLValue($this->wp->GetDBValue("3"), ccsText) . "%'\n" .
        "AND tipo_periodo LIKE '%" . $this->SQLValue($this->wp->GetDBValue("4"), ccsText) . "%' ";
...
class clsRecordperiodos_cargaSearch { //periodos_cargaSearch Class @38-87C75BE3
...
//Class_Initialize Event @38-4D0D8602
    function clsRecordperiodos_cargaSearch($RelativePath, & $Parent)
    {
        ...
        if($this->Visible)
        {
            ...
            $this->s_tipo_periodo = new clsControl(ccsListBox, "s_tipo_periodo", "Tipo Periodo", ccsText, "", CCGetRequestParam("s_tipo_periodo", $Method, NULL), $this);
            $this->s_tipo_periodo->DSType = dsListOfValues;
            $this->s_tipo_periodo->Values = array(array("Normal", "Normal"), array("Extraordinario", "Extraordinario"));
        }
    }
//End Class_Initialize Event

//Validate Method @38-2A38A897
    function Validate()
    {
        global $CCSLocales;
        $Validation = true;
...
//CheckErrors Method @38-D2464CF9
    ...
    function Operation()
    {
        ...
        if($this->Validate()) {
            if($this->PressedButton == "Button_DoSearch") {
                $Redirect = "periodos.php" . "?" . CCMergeQueryStrings(CCGetQueryString("Form", array("Button_DoSearch", "Button_DoSearch_x", "Button_DoSearch_y")));
...
} //End periodos_cargaSearch Class @38-FCB6E20C

class clsRecordperiodos_carga1 { //periodos_carga1 Class @45-53EC4D85
...
//Class_Initialize Event @45-C4EFBAA0
    function clsRecordperiodos_carga1($RelativePath, & $Parent)
    {
        ...
        if($this->Visible)
        {
            ...
            $this->fecha_limite = new clsControl(ccsTextBox, "fecha_limite", "Fecha Limite", ccsDate, array("mm", "/", "dd", "/", "yyyy", " ", "h", ":", "nn", " ", "AM/PM"), CCGetRequestParam("fecha_limite", $Method, NULL), $this);
            $this->fecha_limite->Required = true;
            if(!$this->FormSubmitted) {
                if(!is_array($this->fecha_limite->Value) && !strlen($this->fecha_limite->Value) && $this->fecha_limite->Value !== false)
                    $this->fecha_limite->SetValue(time());
            }
        }
    }
//End Class_Initialize Event

//Initialize Method @45-777CABE1
    function Initialize()
    {
...
//CheckErrors Method @45-ACC1FE21
    function CheckErrors()
    {
        $errors = false;
...
} //End periodos_carga1 Class @45-FCB6E20C

class clsperiodos_carga1DataSource extends clsDBcon_xls {  //periodos_carga1DataSource Class @45-9F90EC5C

//DataSource Variables @45-A28D4A1C
    public $Parent = "";
    public $CCSEvents = "";
    public $CCSEventResult;
    public $ErrorBlock;
...
//DataSourceClass_Initialize Event @45-796EA96F
...
} //End periodos_carga1DataSource Class @45-FCB6E20C

//Initialize Page @1-1086188D
// Variables
$FileName = "";
$Redirect = "";
$Tpl = "";
