<?php
//Include Common Files @1-B8304506
define("RelativePath", ".");
define("PathToCurrentPage", "/");
define("FileName", "usuario_maint.php");
...
class clsRecordusuario { //usuario Class @2-43EF6061
...
//Class_Initialize Event @2-6788AA9C
    function clsRecordusuario($RelativePath, & $Parent)
    {
        ...
        $this->ErrorBlock = "Record usuario/Error";
        ...
        if($this->Visible)
        {
            ...
            $this->nombre_usuario = new clsControl(ccsTextBox, "nombre_usuario", "Nombre Usuario", ccsText, "", CCGetRequestParam("nombre_usuario", $Method, NULL), $this);
            $this->nombre_usuario->Required = true;
            $this->usuario1 = new clsControl(ccsTextBox, "usuario1", "Usuario", ccsText, "", CCGetRequestParam("usuario1", $Method, NULL), $this);
            $this->usuario1->Required = true;
            $this->password = new clsControl(ccsTextBox, "password", "Password", ccsText, "", CCGetRequestParam("password", $Method, NULL), $this);
            $this->password->Required = true;
            $this->nivel = new clsControl(ccsTextBox, "nivel", "Nivel", ccsInteger, "", CCGetRequestParam("nivel", $Method, NULL), $this);
            $this->nivel->Required = true;
        }
    }
//End Class_Initialize Event

//Initialize Method @2-8552E899
    function Initialize()
    {
        ...
        $this->DataSource->Parameters["urlid_usuario"] = CCGetFromGet("id_usuario", NULL);
    }
//End Initialize Method

//Validate Method @2-1BC6EEF2
    function Validate()
    {
        global $CCSLocales;
        $Validation = true;
        $Where = "";
        $Validation = ($this->nombre_usuario->Validate() && $Validation);
        $Validation = ($this->usuario1->Validate() && $Validation);
        $Validation = ($this->password->Validate() && $Validation);
        $Validation = ($this->nivel->Validate() && $Validation);
        $this->CCSEventResult = CCGetEvent($this->CCSEvents, "OnValidate", $this);
        $Validation =  $Validation && ($this->nombre_usuario->Errors->Count() == 0);
        $Validation =  $Validation && ($this->usuario1->Errors->Count() == 0);
...
//CheckErrors Method @2-EB0DAD92
    function CheckErrors()
    {
        $errors = false;
        $errors = ($errors || $this->nombre_usuario->Errors->Count());
        $errors = ($errors || $this->usuario1->Errors->Count());
...
//Operation Method @2-5A829E97
    function Operation()
    {
        ...
        $Redirect = "usuario_list.php" . "?" . CCGetQueryString("QueryString", array("ccsForm"));
...
//InsertRow Method @2-337CA4BE
    function InsertRow()
    {
        $this->CCSEventResult = CCGetEvent($this->CCSEvents, "BeforeInsert", $this);
        if(!$this->InsertAllowed) return false;
        $this->DataSource->nombre_usuario->SetValue($this->nombre_usuario->GetValue(true));
        $this->DataSource->usuario1->SetValue($this->usuario1->GetValue(true));
...
//Show Method @2-16CC1A94
    function Show()
    {
        ...
        if($this->FormSubmitted || $this->CheckErrors()) {
            $Error = "";
            $Error = ComposeStrings($Error, $this->nombre_usuario->Errors->ToString());
            $Error = ComposeStrings($Error, $this->usuario1->Errors->ToString());
...
} //End usuario Class @2-FCB6E20C

class clsusuarioDataSource extends clsDBcon_xls {  //usuarioDataSource Class @2-A0B5C18B

//DataSource Variables @2-39A0D301
    public $Parent = "";
    public $CCSEvents = "";
    public $CCSEventResult;
    public $ErrorBlock;
...
//DataSourceClass_Initialize Event @2-57A389BD
    function clsusuarioDataSource(& $Parent)
    {
        ...
        $this->InsertFields["nombre_usuario"] = array("Name" => "nombre_usuario", "Value" => "", "DataType" => ccsText, "OmitIfEmpty" => 1);
        ...
        $this->UpdateFields["nombre_usuario"] = array("Name" => "nombre_usuario", "Value" => "", "DataType" => ccsText, "OmitIfEmpty" => 1);
...
//Prepare Method @2-8226CDF6
    function Prepare()
    {
        global $CCSLocales;
        global $DefaultDateFormat;
        $this->wp = new clsSQLParameters($this->ErrorBlock);
        $this->wp->AddParameter("1", "urlid_usuario", ccsInteger, "", "", $this->Parameters["urlid_usuario"], "", false);
...
//Insert Method @2-812EC00D
    function Insert()
    {
        ...
        $this->InsertFields["nombre_usuario"]["Value"] = $this->nombre_usuario->GetDBValue(true);
        $this->InsertFields["usuario"]["Value"] = $this->usuario1->GetDBValue(true);
        $this->InsertFields["password"]["Value"] = $this->password->GetDBValue(true);
        $this->InsertFields["nivel"]["Value"] = $this->nivel->GetDBValue(true);
        $this->SQL = CCBuildInsert("usuario", $this->InsertFields, $this);
...
//Update Method @2-4F945BEF
    function Update()
    {
        ...
        $this->UpdateFields["nombre_usuario"]["Value"] = $this->nombre_usuario->GetDBValue(true);
        $this->UpdateFields["usuario"]["Value"] = $this->usuario1->GetDBValue(true);
        $this->UpdateFields["password"]["Value"] = $this->password->GetDBValue(true);
        $this->UpdateFields["nivel"]["Value"] = $this->nivel->GetDBValue(true);
        $this->SQL = CCBuildUpdate("usuario", $this->UpdateFields, $this);
...
} //End usuarioDataSource Class @2-FCB6E20C

//Include Page implementation @17-07AA2166
include_once(RelativePath . "/MenuIncludablePage.php");
//End Include Page implementation

//Initialize Page @1-3D0B75A7
// Variables
$FileName = "";
$Redirect = "";
$Tpl = "";
...
$FileName = FileName;
$Redirect = "";
$TemplateFileName = "usuario_maint.html";
$BlockToParse = "main";
...
$usuario = new clsRecordusuario("", $MainPage);
...
//Execute Components @1-FFAF3EFE
$MasterPage->Operations();
$MenuIncludablePage->Operations();
$usuario->Operation();
//End Execute Components

//Go to destination page @1-D8D25579
if($Redirect)
{
