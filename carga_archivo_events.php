     ...
     if ($NewRecord1->id_reg_ok->GetValue()>0)
     {
	      ...
	      ///////////////7
	    	...
	    	$id_proveedor=CCDLookUp("id_proveedor","usuario","id_usuario=".$conx_xls->ToSQL($id_usuario, ccsInteger),   $conx_xls);
	      	$id_grupo=CCDLookUp("id_grupo","grupo_proveedor","id_proveedor=".$conx_xls->ToSQL($id_proveedor, ccsInteger),   $conx_xls);
	       //////
	       ...
	       $str_fecha_limite=CCDLookUp("fecha_limite","periodos_carga","id_periodo=".$conx_xls->ToSQL($id_periodo, ccsInteger),   $conx_xls);
	     	$vigencia=ComparaFechasYMD($str_fecha_limite);
...
function ValidaInfoCargada($id_proveedor,$id_registro )
{
	...
	$ssql = "Select count(*) as num_incidentes from l_calificacion_incidentes_AUT where id_proveedor=$id_proveedor and id_registro=$id_registro "; 
	//echo $ssql;
	$db->query($ssql);
	while ($db->next_record() ) {  		         
  	...
  	$ssql = "Select count(*) as num_inc_ns from l_detalle_medicion_inc where id_proveedor=$id_proveedor and id_registro=$id_registro";
      	//echo $ssql;
    	$db->query($ssql);
    	while ($db->next_record() ) {  		         
	        $num_inc_ns=$db->f(0);	 	       
	...
	//validacion de ids
	$ssql="Select count(a.id_incidencia) from l_calificacion_incidentes_AUT a, l_detalle_medicion_inc b
	...
	$ssql="Select count(*) from l_calificacion_incidentes_AUT where id_proveedor=$id_proveedor and id_registro=$id_registro
	...
	$ssql="Select count(*) from l_detalle_medicion_inc where id_proveedor=$id_proveedor and id_registro=$id_registro
	    		and id_incidencia like 'INC%'";
	...
	$ssql="Select distinct id_ppmc from  l_detalle_medicion_apertura  where id_proveedor=$id_proveedor and id_registro=$id_registro ";
	...
	$ssql="Select distinct id_ppmc from  l_detalle_medicion_cierre  where id_proveedor=$id_proveedor and id_registro=$id_registro ";
	...
	$ssql="Select distinct (c.id_ppmc) from  l_detalle_medicion_cierre c, l_calificacion_rs_AUT t
	...
	$ssql="Select distinct (a.id_ppmc) from  l_detalle_medicion_apertura a, l_calificacion_rs_AUT t
	  where a.id_proveedor=$id_proveedor and a.id_registro=$id_registro
	...
	$ssql="Select id_incidencia, count(id_incidencia)as total from l_calificacion_incidentes_AUT 
      ...
      $ssql="Select id_incidencia, count(id_incidencia)as total from l_detalle_medicion_inc 
		where id_proveedor= $id_proveedor  and id_registro=$id_registro
 		group by id_incidencia order by total desc		 ";
